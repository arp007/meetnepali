...
google.devrel.samples.hello.CLIENT_ID =
    '245777491056-ppjmr2kjk8mbkoo3ra4civmma7hkn0cd.apps.googleusercontent.com';
...
google.devrel.samples.hello.print = function(greeting) {
  ...
  element.innerHTML = greeting.email.email;
...
google.devrel.samples.hello.getUser = function(email) {
	  gapi.client.helloworld.user.getUserAccount({'email': email}).execute(
	      function(resp) {
	    	  console.log(resp);
	        if (!resp.code) {
	          console.log(resp);
...
google.devrel.samples.hello.enableButtons = function() {
  document.getElementById('getGreeting').onclick = function() {
    google.devrel.samples.hello.getUser(
